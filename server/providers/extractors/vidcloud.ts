    ...
    try {
      ...
      try {
        const { data } = await axios.get(masterPlaylistUrl, {
          headers: { Referer: videoUrl.href, 'User-Agent': USER_AGENT },
          timeout: 10000,
        })
        masterPlaylist = data
      } catch {
        ...
        try {
          ...
          return {
            ...
            subtitles: initialData.tracks?.map((s: any) => ({
              url: s.file,
      ...
      sources.push({ url: masterPlaylistUrl, isM3U8: true, quality: 'auto' })
      ...
      while ((match = playlistRegex.exec(masterPlaylist!)) !== null) {
        ...
        sources.push({ url, quality, isM3U8: url.includes('.m3u8') })
      }

      result.sources = sources
      result.subtitles = initialData.tracks?.map((s: any) => ({
        url: s.file,
        lang: s.label ?? 'Default',
      })) || []
