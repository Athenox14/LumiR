...
class TMDB extends MovieParser {
  ...
  fetchTrending = async (
    ...
    try {
      ...
      result.results = data.results.map((item: any) => {
        if (item.media_type !== 'person') {
          ...
          user.movies = item['known_for'].map((movie: any) => {
    ...
    try {
      ...
      info.similar =
        data?.similar?.results?.length <= 0
          ? undefined
          : data?.similar?.results.map((item: any) => {
              return {
                id: item.id,
                title: item.title || item.name,
                image: `/api/images/original${item.poster_path}`,
            ...
            seasonData?.episodes?.length <= 0
              ...
              : seasonData?.episodes.map((episode: any): IMovieEpisode => {
                  const episodeFromProvider = seasonEpisodes?.find(
                    (ep: any) => ep.number === episode.episode_number
                  )
